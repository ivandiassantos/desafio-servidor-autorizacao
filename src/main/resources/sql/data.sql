...
INSERT INTO user (username, email, password, activated)
SELECT * FROM (SELECT 'cliente', 'cliente@cliente.com', '$2a$10$r0RFDmpneBVryx.ihHK9gu6FFJQi4nTxQUqzdSTvrPpaKZMxigqpy', true) AS tmp
WHERE NOT EXISTS (
    SELECT username FROM user WHERE username = 'cliente'
) LIMIT 1;
...
SELECT * FROM (SELECT 'cliente', 'ROLE_USER') AS tmp
WHERE NOT EXISTS (
    SELECT username, authority FROM user_authority WHERE username = 'cliente' and authority = 'ROLE_USER'
) LIMIT 1;
